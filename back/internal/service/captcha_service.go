// ============================================================
// 文件：back/internal/service/captcha_service.go
// 作用：验证码登录流程的业务逻辑（发送邮箱验证码、验证并登录）。
//
// 验证码登录流程：
//   1. 用户输入邮箱，前端调用 /captcha/send_email
//   2. 后端生成 6 位随机数字验证码，通过 SMTP 发送到邮箱
//   3. 同时把验证码存入 Redis（key="captcha:邮箱地址"，TTL=5分钟）
//   4. 用户收到邮件后，在前端输入验证码
//   5. 前端调用 /captcha/login_email，后端从 Redis 取出正确验证码比对
//   6. 比对成功则签发 JWT token，登录完成；同时删除 Redis 里的验证码（防止重复使用）
//
// 安全考虑：
//   - 验证码 5 分钟过期，超时需重新发送
//   - 验证成功后立即删除 Redis 里的验证码（单次使用）
//   - 验证码是 6 位数字，共 1,000,000 种可能，短时间内暴力破解概率极低
//   - 生产环境可以加速率限制（如同一邮箱 1 分钟内只能发一次）
// ============================================================
package service
