// ============================================================
// 文件：back/internal/controller/v1/message.go
// 作用：消息相关的 HTTP handler：历史消息查询、撤回、标记已读、清空会话。
//
// 注意这里没有"发消息"的接口：
//   消息发送走的是 WebSocket（/wss），不是 HTTP。
//   HTTP 接口只负责"查询历史"、"修改状态"这类不需要实时性的操作。
//   这种设计分离了"实时通信（WebSocket）"和"数据查询（HTTP REST）"两种职责。
// ============================================================
package v1
