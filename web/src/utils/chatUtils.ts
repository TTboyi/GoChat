// ============================================================
// 文件：web/src/utils/chatUtils.ts
// 作用：前端聊天相关的纯工具函数集合（localStorage 缓存、路径工具、备注管理）。
//
// 为什么要把消息缓存到 localStorage（saveMessagesToStorage）？
//   聊天消息数据来自后端 API 和 WebSocket，但这些数据存在内存里，
//   页面一刷新就全丢了，用户每次刷新都要等待重新加载。
//   localStorage 持久化后，页面刷新时能立刻展示上次的聊天记录，
//   等新数据加载完再合并更新，体验更流畅。
//
// 备注（remarks）机制：
//   用户可以给好友设置本地备注名（如"老板"代替真实昵称）。
//   备注只存在 localStorage，不同步到服务器，刷新/重开标签页都保留。
// ============================================================
